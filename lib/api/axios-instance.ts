...
axiosInstance.interceptors.request.use(
  (config) => {
    // Note: We cannot manually set Cookie header - browsers block it as "unsafe header"
    // Cookies must be set by backend via Set-Cookie header and sent automatically by browser
    // with credentials: 'include' (which is set via withCredentials: true)
    ...
    if (typeof window !== 'undefined' && (window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1')) {
      console.log('📤 Client API Request:', {
        ...
        note: 'Cookie will be sent automatically by browser if set by backend'
      });
    }
    return config;
  },
