    ...
    async (config) => {
      ...
      if (token && config.headers) {
        ...
        console.error('🔐 Adding authentication cookie to request:', {
          ...
          cookieHeaderSet: !!config.headers.Cookie,
...
export async function importServiceRequests(
  ...
  try {
    ...
    const endpointWithParams = preventAutoConvert
      ? `${endpoint}?autoConvert=false&createBookings=false&keepAsPending=true`
      : endpoint;
    ...
    console.log('📤 Import request parameters (preventing auto-conversion):', {
      ...
      expectedBehavior:
        'Backend should save imported records as service requests only. They will NOT be converted to bookings until auto-dispatch is run.',
    });
